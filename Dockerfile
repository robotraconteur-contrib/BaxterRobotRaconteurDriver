...
COPY ./BaxterRobotRaconteurDriver /app/BaxterRobotRaconteurDriver
RUN cd BaxterRobotRaconteurDriver \
    && dotnet publish --framework=net6.0 --self-contained false -o /opt/baxter_robotraconteur_driver/bin

COPY ./BaxterRobotRaconteurDriver/config/*.yml /config/

RUN rm -rf /app && apt-get clean && rm -rf /var/lib/apt/lists/*


ENV ROBOT_INFO_FILE=/config/baxter_both_arm_robot_default_config.yml
ENV ROS_MASTER_URI=http://127.0.0.1:11311
ENV ROS_IP=127.0.0.1

CMD exec /opt/baxter_robotraconteur_driver/bin/BaxterRobotRaconteurDriver --robot-info-file=$ROBOT_INFO_FILE